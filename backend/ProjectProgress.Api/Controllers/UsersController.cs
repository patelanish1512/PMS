...
public class UsersController : ControllerBase
{
    ...
    public async Task<ActionResult<List<object>>> GetUsers()
    {
        ...
        var teamData = users.Select(u => {
            var userTasks = tasks.Where(t => t.AssignedUserIds.Contains(u.Id) || (t.AssigneeNames != null && t.AssigneeNames.Contains(u.FullName))).ToList();
            ...
            return new {
                ...
                tasksCompleted = userTasks.Count(t => t.Status == Domain.Enums.TaskStatus.Done),
    ...
    public async Task<ActionResult> CreateUser([FromBody] Domain.Entities.User user)
    {
    ...
    [HttpPut("{id}")]
    [Authorize(Roles = "Admin")]
    public async Task<ActionResult> UpdateUser(string id, [FromBody] Domain.Entities.User user)
    {
    ...
    public async Task<ActionResult> UpdateRole(string id, [FromBody] string role)
    {
        ...
        var update = Builders<Domain.Entities.User>.Update.Set(u => u.Role, userRole);
    ...
    {
        var update = Builders<Domain.Entities.User>.Update.Set(u => u.Status, "inactive");
        var result = await _context.Users.UpdateOneAsync(u => u.Id == id, update);
        if (result.MatchedCount == 0) return NotFound();
        return Ok();
    }
    ...
    {
        var update = Builders<Domain.Entities.User>.Update.Set(u => u.Status, status);
        var result = await _context.Users.UpdateOneAsync(u => u.Id == id, update);
        if (result.MatchedCount == 0) return NotFound();
        return Ok();
    }
    ...
    public async Task<ActionResult> ChangePassword([FromBody] PasswordChangeDto dto)
    {
        ...
        var update = Builders<Domain.Entities.User>.Update.Set(u => u.PasswordHash, newHash);
        await _context.Users.UpdateOneAsync(u => u.Id == userId, update);
